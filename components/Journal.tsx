        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {JOURNAL_ARTICLES.map((article) => (
                <div key={article.id} className="group cursor-pointer flex flex-col text-left" onClick={() => onArticleClick(article)}>
                    <div className="w-full aspect-[4/3] overflow-hidden mb-8 bg-[#EBE7DE] rounded-2xl shadow-sm">
                        <img 
                            src={article.image} 
                            alt={article.title} 
                    ...
                    <div className="flex flex-col flex-1 text-left">
                        <span className="text-xs font-bold uppercase tracking-widest text-[#DAA520] mb-3">{article.date}</span>
                        <h3 className="text-2xl font-serif text-[#1A3C34] mb-4 leading-tight group-hover:text-[#DAA520] transition-colors">{article.title}</h3>
